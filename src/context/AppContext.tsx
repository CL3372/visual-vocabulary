...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  // Native iOS/Android Pro status comes from StoreKit / Google Play Billing
  // via RevenueCat, not Stripe — see src/hooks/useIAP.ts for why (App Store
  // Guideline 3.1.1 and the equivalent Play policy). On the web build
  // isNativeMobile is false and this is a no-op.
  const { isProFromIAP, isNativeMobile, linkIdentity } = useIAP();
  ...
  useEffect(() => {
    if (isNativeMobile && user?.id) linkIdentity(user.id);
  }, [isNativeMobile, user?.id, linkIdentity]);
  ...
  const displayName = (user?.user_metadata?.full_name
    ...
    || localStorage.getItem('vv-display-name')
    || '';
